...
`config/nisa_tsumitate.yaml` に積立設定（開始日・月額・任意でファンド名）を登録すると、
同じセクションに**つみたて投資枠の使用状況**も表示されます。まだ積立を始めていない場合は
ファイルを空（`tsumitate: []`）のままにしておいてください。

```yaml
tsumitate:
  - start_date: "2026-10-01"
    monthly_amount_jpy: 30000
    fund_name: "eMAXIS Slim 全世界株式(オール・カントリー)"  # 任意、表示用
```

表示される内容:

- **年間枠の使用状況**（上限¥1,200,000）と、**生涯投資枠の合計**
  （成長投資枠＋つみたて投資枠、上限¥18,000,000）
- **年内に年間枠を使い切るための月額の目安**（残り枠 ÷ 残り月数の単純計算）。
  まだ積立を始めていない場合は「0円から始めた場合の目安」として表示されます。
  これは「埋めるべき」という推奨ではなく、あくまで参考の目安です
- **つみたて投資枠の対象になりやすいファンドの種類**（全世界株式型・米国株式型・
  先進国株式型・バランス型・国内株式型といった一般的な資産クラスの分類）の簡単な解説。
  特定の商品名を推奨するものではありません

積立額を月の途中で変更・停止した場合は、それまでの行に `end_date` を追加したうえで、
新しい金額の行を別に追加してください（詳しい書式は `nisa_tsumitate.yaml` 内のコメントを
参照してください）。証券口座の正式な消化額とは異なる場合があるため、必ず証券会社側でも
ご確認ください。
...
- 判定を出した日から**30日後**の株価と比較し、判定の方向（上昇を期待/下落を期待）と
  実際の値動きの符号が一致していれば「的中」、逆であれば「不的中」とします
  （もともとは7日後で検証していましたが、プロンプト側で「3ヶ月・6ヶ月・1年の
  中長期トレンドを判断の主たる根拠にする」よう指示しているのに対し7日は短すぎ、
  短期的なノイズで「不正解」と判定されやすかったため、30日に見直しました）
- 変化率が±1%以内の小さな動きは「様子見扱い」として、的中率の集計からは除外します
  （ノイズのような小さな値動きだけで的中・不的中を決めないため）
- 「様子見」「保有継続」（方向性を示さない判定）は、そもそも検証対象にしていません
- 判定から30日経つまでは「検証待ち」として扱われ、まだ的中率には反映されません
...
的中率は表示するだけでなく、以下の4つの切り口でAIへのプロンプトにも渡され、
次回以降の判断（スコアの付け方・確信度）に反映されます。

- **判定タイプ別**（買い候補・売り候補・売却検討）… 5件以上確定した時点から
- **確信度別**（\|スコア\|60以上の「確信度が高い判定」と、それ以外）… 5件以上確定した時点から
- **テーマ別**（分析中の銘柄と同じテーマの過去の的中率。例：「半導体」は過去8件中2件しか
  的中していない、など）… 5件以上確定した時点から
- **銘柄別の連続的中/連続不的中**（例：「この銘柄は直近4回連続で買い判定が外れています」）
  … **2回以上**連続した時点から。同じ銘柄で判定が外れ続けているのに、テーマ別・確信度別の
  平均を見ているだけでは埋もれてしまうケースを直接AIに伝えるための仕組みです

単に「的中率がX%です」と伝えるだけでなく、「確信度が高い判定ほど外れやすい」
「このテーマは外れやすい」「この銘柄は繰り返し外れている」といった、より具体的な
不正解の傾向をAIに伝えることで、判断の重み付けを見直しやすくする狙いです。
1つの銘柄について前回の判断だけを振り返る既存の「反省機能」とは別に、判定タイプ・
確信度・テーマ・銘柄全体としての傾向をAI自身にフィードバックする仕組みです。
件数がまだ少ないうちはノイズが大きいため、判定タイプ別・確信度別・テーマ別は
5件未満、銘柄別の連続記録は2回未満の場合はプロンプトに含めません。

確信度の閾値はもともと\|スコア\|80以上でしたが、実際にAIが付けるスコアは25〜65程度に
収まることが多く、80では「確信度が高い判定」がほぼ出現せず区分が機能していなかったため、
60に引き下げました（`scripts/track_record.py`の`HIGH_CONFIDENCE_ABS_SCORE`）。

### 「確信度が高いほど外れる」問題への対処（2026-09-15）

実際のデータを確認したところ、\|スコア\|60以上の「確信度が高い判定」の的中率が0%
（9件中0件的中）で、通常の確信度の判定（的中率約21%）よりもさらに低いという、
確信度が実際の正しさと逆転している状態が見つかりました。これは「直近の短期的な
値上がり・値下がりの勢いが強い」ことを理由に高いスコアを付け、その後の中長期では
値動きが反転する（モメンタムの過信・平均回帰）ことが一因と考えられます。

これに対して2つの対策を入れています。

- **確信度の逆転を検知したら、プロンプトで明示的に警告する**: 「確信度が高い判定」の
  的中率が「通常の確信度の判定」を下回っている場合、通常の傾向フィードバックとは別に
  「確信度スコアが実際の正しさを反映できていない」という警告文をAIに伝えます
  （`scripts/analyze.py`の`_build_accuracy_section`）
- **システムプロンプトに恒久的なルールとして追加する**: 十分な件数が確定するまでは
  動的なフィードバックが表示されないため、それとは別に「複数の独立した根拠が同じ方向を
  示している場合に限り高いスコアを付けること」「根拠が弱ければ様子見を選ぶこと」を
  常時のルールとしてシステムプロンプトに明記しました

またあわせて、的中率が「50%未満なら外れやすい」という1段階の評価だけだと、
本当に悪い状態（偶然より明確に悪い）なのか、ただの平均以下なのかが区別できなかったため、
35%未満は「偶然（五分五分）よりも明確に悪い」というより強い警告文に分けました
（`scripts/analyze.py`の`_describe_trend`）。

### 改善施策後の的中率（改善が効いているかを早めに確認する仕組み）

判定は記録から確定まで30日かかるため、上記のような改善策を導入しても、その効果は
全期間の的中率にはすぐには反映されません（改善前に記録された判定がまだ大量に
残っているため）。そこで、検証期間の延長・確信度閾値の見直し・銘柄別連続記録の
導入日（2026-09-08）以降に**記録**された判定だけの的中率も別途集計し、
レポートの「AIの的中率」セクションに表示しています
（`scripts/track_record.py`の`_IMPROVEMENT_COHORT_START`）。
今後、新しい対策を追加した場合もこの日付を更新することで、同じ仕組みで
「対策が効いているか」を確認できます。

### シミュレーション損益（判定通りに行動していた場合）

「的中したか外れたか」という二値の統計だけでは実際の損益感覚と結びつきにくいため、
判定が確定するたびに「もしこの判定通りに行動していたら、リターンは何%だったか」も
あわせて集計し、レポートの「AIの的中率」セクションに表示しています。

- **買い候補**: 判定時に買って30日後に売った場合の値上がり率をそのまま使用
- **売り候補・売却検討**: 判定時に売った/買わなかった場合に得をした割合
  （実際の値動きの符号を反転させたもの）を使用
- 等金額・単利（複利しない）で計算し、手数料や税金は考慮していません
- 判定種別ごとの平均リターンと勝敗数、および全シグナルを通した平均リターンを表示します

あくまで「AIの判定を機械的に採用し続けたら平均してどうだったか」という振り返り指標であり、
個別の売買を推奨するものではありません。データは`data/track_record.json`の
`simulated_pl`に集計され、`recent_resolved`の各件にも仮想リターンが記録されます。
