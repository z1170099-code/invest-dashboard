...
1. `config/watchlist.yaml` に登録した銘柄（自分専用）と、`config/candidate_pool.yaml`
   に登録したテーマ別のハイリスク候補銘柄について、株価（yfinance）と関連ニュース
   （Google News RSS検索）、世界情勢・マクロ経済ニュースを取得する
2. Gemini APIがそれらを読み、銘柄ごとにスコア・推奨・理由を生成する
   （世界情勢・マクロ経済ニュースも判断材料としてプロンプトに含めている）
3. 結果を `docs/index.html` という1枚のHTMLファイルにまとめる。レポートは
   「①自分の銘柄ランキング」「②世界情勢から注目したハイリスク・テーマ銘柄」の
   2セクション構成になる
