...
3. 結果を `docs/index.html` という1枚のHTMLファイルにまとめる。レポートは
   「①AIによる売買サマリー」「②AIの的中率」「③保有銘柄の売却タイミング」「④暗号資産」
...
以下の2つのケースで、Gmail経由でメール通知を送る機能です。両方同時に該当する日は、
2通に分かれず1通にまとめて届きます。

- 保有銘柄が**新たに**「売却検討」と判定されたとき
- ウォッチリスト・ハイリスク候補銘柄（暗号資産含む）が**新たに**スコア70以上の
  「買い候補」になったとき（「AIによる売買サマリー」の買い候補欄と同じ対象範囲です）

どちらも「前回はそうではなかったが今回そうなった」銘柄だけが対象です。既にその状態のまま
何日も経過している銘柄については毎日は通知しません（同じ内容を送り続けて
煩わしくならないようにするためです）。スコア70という基準は`scripts/notify.py`の
`_STRONG_BUY_SCORE_THRESHOLD`で変更できます。
...
的中率は表示するだけでなく、以下の3つの切り口で**5件以上**確定した時点から、
AIへのプロンプトにも「あなたの過去の『買い候補』判定はX件中Y件的中しています」という
形で渡され、次回以降の判断（スコアの付け方・確信度）に反映されます。

- **判定タイプ別**（買い候補・売り候補・売却検討）
- **確信度別**（\|スコア\|80以上の「確信度が高い判定」と、それ以外）
- **テーマ別**（分析中の銘柄と同じテーマの過去の的中率。例：「半導体」は過去8件中2件しか
  的中していない、など）

単に「的中率がX%です」と伝えるだけでなく、「確信度が高い判定ほど外れやすい」
「このテーマは外れやすい」といった、より具体的な不正解の傾向をAIに伝えることで、
判断の重み付けを見直しやすくする狙いです。1つの銘柄について前回の判断だけを
振り返る既存の「反省機能」とは別に、判定タイプ・確信度・テーマ全体としての
傾向をAI自身にフィードバックする仕組みです。
件数がまだ少ないうちはノイズが大きいため、5件未満の内訳はプロンプトに含めません。
